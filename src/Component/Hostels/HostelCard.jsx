...
const HostelCard = ({ hostel }) => {
  ...
  // Create a dynamic link based on hostel name, formatted to remove spaces
  const hostelLink = `/hosteldetail/${encodeURIComponent(
    hostel.name.toLowerCase().replace(/\s+/g, "-")
  )}`;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      <div className="mt-4">
        ...
        <p className="text-sm text-[#2C3E50] font-[poppins] mt-1">
          {hostel.location}
        </p>
        <p className="text-sm text-[#2C3E50] font-[poppins] mt-1">
          Price: NPR {hostel.price}/month
        </p>
        <p className="text-sm text-[#2C3E50] font-[poppins] mt-1">
          {hostel.description}
        </p>
        ...
        <div className="flex items-center justify-start mt-4 space-x-2 text-sm font-[poppins] text-[#2C3E50]">
          {hostel.facilities?.map((facility, index) => (
            <span key={index} className="bg-[#E8F8F5] p-1 rounded">
              {facility}
            </span>
